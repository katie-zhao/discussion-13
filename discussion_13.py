...
def create_employee_table(cur, conn):
    cur.execute('''CREATE TABLE IF NOT EXISTS Employees (employee_id INTEGER, first_name TEXT
                last_name TEXT, job_id INTEGER PRIMARY KEY, hire_date INTEGER, salary INTEGER)''')
    conn.commit()
...
def add_employee(filename, cur, conn):
    ...
    employee_list = json.loads(file_data)
    ...
    for employee in employee_list:
        ...
        cur.execute("INSERT OR IGNORE INTO Employees VALUES (?,?,?,?,?,?)",(employee_id, first_name,
...
def job_and_hire_date(cur, conn):
    cur.execute('SELECT Employees.hire_date, Jobs.job_title FROM Employees JOIN Jobs ON Employees.job_id = Jobs.job_id ORDER BY hire_date')
